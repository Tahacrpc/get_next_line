...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/12/28 23:52:05 by tcirpici          #+#    #+#             */
/*   Updated: 2024/12/29 00:27:46 by tcirpici         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE <= 0 || fd > 256)
		return (0);
	stack[fd] = read_gnl(fd, stack[fd]);
	if (!stack[fd])
		return (0);
	line = line_gnl(stack[fd]);
	stack[fd] = trim_gnl(stack[fd]);
	return (line);
}
